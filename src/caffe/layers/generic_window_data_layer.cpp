...
void GenericWindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // LayerSetUp runs through the data_file and creates two structures
  // that hold image regions and multiple labels.

  // data_file format
  // 2 * channels (RGB or gray image)
  // repeated:
  //    # image_index
  //    img_path_1 (abs path)
  //    img_path_2 (abs path)
  //    x1 y1 x2 y2(the left-top and right-bottom positions of image region)
  //    x_trans_label
  //    z_trans_label
  //    y_rotation_label
  ...
  //     << "  foreground (object) overlap threshold: "
      // << this->layer_param_.generic_window_data_param().fg_threshold() << std::endl
      // << "  background (non-object) overlap threshold: "
      // << this->layer_param_.generic_window_data_param().bg_threshold() << std::endl
      // << "  foreground sampling fraction: "
      // << this->layer_param_.generic_window_data_param().fg_fraction() << std::endl
      // << "  cache_images: "
  ...
  do {
    CHECK_EQ(hashtag, "#");
    // read image path
    string image_path_1, image_path_2;
    infile >> image_path_1 >> image_path_2;
    image_path_1 = root_folder + image_path_1;
    image_path_2 = root_folder + image_path_2;
    image_database_.push_back(std::make_pair(image_path_1, image_path_2));
    ...
    //     this->layer_param_.generic_window_data_param().fg_threshold();
    // const float bg_threshold =
    //     this->layer_param_.generic_window_data_param().bg_threshold();
    ...
    //   }
    // }
    int x1, y1, x2, y2, x_trans_label, z_trans_label, y_rotation_label;
    infile >> x1 >> y1 >> x2 >> y2 
        >> x_trans_label >> z_trans_label >> y_rotation_label;
    vector<float> window(GenericWindowDataLayer::NUM);
    window[GenericWindowDataLayer::IMAGE_INDEX] = image_index;
    window[GenericWindowDataLayer::X1] = x1;
    window[GenericWindowDataLayer::Y1] = y1;
    window[GenericWindowDataLayer::X2] = x2;
    window[GenericWindowDataLayer::Y2] = y2;
    window[GenericWindowDataLayer::X_TRANS_LABEL] = x_trans_label;
    window[GenericWindowDataLayer::Z_TRANS_LABEL] = z_trans_label;
    window[GenericWindowDataLayer::Y_ROTAION_LABEL] = y_rotation_label;

    image_windows_.push_back(window);
  ...
  LOG(INFO) << "output data size: " << top[0]->num() << ","
      ...
      << top[0]->width();
  // labels
  const int num_labels = GenericWindowDataLayer::NUM - 5;
  vector<int> label_shape(1, batch_size);
  label_shape.push_back(num_labels);
...
void GenericWindowDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  //     this->layer_param_.generic_window_data_param().fg_fraction();
  Dtype* mean = NULL;
  // int mean_off = 0;
  int mean_width = 0;
  int mean_height = 0;
  ...
  //         image_database_[window[GenericWindowDataLayer<Dtype>::IMAGE_INDEX]];

  //     cv::Mat cv_img;
  //     if (this->cache_images_) {
  //       pair<std::string, Datum> image_cached =
  //         image_database_cache_[window[GenericWindowDataLayer<Dtype>::IMAGE_INDEX]];
  ...
  //     int x1 = window[GenericWindowDataLayer<Dtype>::X1];
  //     int y1 = window[GenericWindowDataLayer<Dtype>::Y1];
  //     int x2 = window[GenericWindowDataLayer<Dtype>::X2];
  //     int y2 = window[GenericWindowDataLayer<Dtype>::Y2];
  ...
  //     // get window label
  //     top_label[item_id] = window[GenericWindowDataLayer<Dtype>::LABEL];
  ...
  //         << window[GenericWindowDataLayer<Dtype>::X1]+1 << std::endl
  //         << window[GenericWindowDataLayer<Dtype>::Y1]+1 << std::endl
  //         << window[GenericWindowDataLayer<Dtype>::X2]+1 << std::endl
  //         << window[GenericWindowDataLayer<Dtype>::Y2]+1 << std::endl
  //         << do_mirror << std::endl
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    const unsigned int rand_index = PrefetchRand();
    vector<float> window = image_windows_[rand_index % image_windows_.size()];

    bool do_mirror = mirror && PrefetchRand() % 2;
    // load the image containing the window
    pair<std::string, std::string > image =
    image_database_[window[GenericWindowDataLayer<Dtype>::IMAGE_INDEX]];

    cv::Mat cv_img_1;
    cv::Mat cv_img_2;

    cv_img_1 = cv::imread(image.first, CV_LOAD_IMAGE_COLOR);
    cv_img_2 = cv::imread(image.second, CV_LOAD_IMAGE_COLOR);
    // cv::Size cv_size(image_width, image_height);
    // cv::resize(cv_img_1, cv_img_1, cv_size, 0, 0, cv::INTER_LINEAR);
    // cv::resize(cv_img_2, cv_img_2, cv_size, 0, 0, cv::INTER_LINEAR);
    ...
    if (!cv_img_2.data) {
      LOG(ERROR) << "Could not open or find file " << image.second;
      return;
    }
    read_time += timer.MicroSeconds();
    timer.Start();

    CHECK_EQ(cv_img_1.channels(), cv_img_2.channels());
    const int channels = cv_img_1.channels();
    ...
    if (context_pad > 0 || use_square) {
      ...
      int pad_x1 = std::max(0, -x1);
      int pad_y1 = std::max(0, -y1);
      int pad_x2 = std::max(0, x2 - cv_img_1.cols + 1);
      int pad_y2 = std::max(0, y2 - cv_img_1.rows + 1);
      // clip bounds
      x1 = x1 + pad_x1;
      x2 = x2 - pad_x2;
      y1 = y1 + pad_y1;
      ...
      CHECK_GT(x1, -1);
      CHECK_GT(y1, -1);
      CHECK_LT(x2, cv_img_1.cols);
      CHECK_LT(y2, cv_img_1.rows);
    ...
    cv::Rect roi(x1, y1, x2-x1+1, y2-y1+1);
    cv::Mat cv_cropped_img_1 = cv_img_1(roi);
    // cv::resize(cv_cropped_img_1, cv_cropped_img_1,
    //     cv_crop_size, 0, 0, cv::INTER_LINEAR);

    cv::Mat cv_cropped_img_2 = cv_img_2(roi);
    // cv::resize(cv_cropped_img_2, cv_cropped_img_2,
    //     cv_crop_size, 0, 0, cv::INTER_LINEAR);

    // horizontal flip at random
    if (do_mirror) {
      cv::flip(cv_cropped_img_1, cv_cropped_img_1, 1);
      cv::flip(cv_cropped_img_2, cv_cropped_img_2, 1);
    }

    // copy the warped window into top_data
    for (int h = 0; h < cv_cropped_img_1.rows; ++h) {
      const uchar* ptr_1 = cv_cropped_img_1.ptr<uchar>(h);
      const uchar* ptr_2 = cv_cropped_img_2.ptr<uchar>(h);
      int img_index = 0;
      for (int w = 0; w < cv_cropped_img_1.cols; ++w) {
        for (int c = 0; c < channels; ++c) {
          int top_index_1 = (((item_id * 2) * channels + c) * crop_size + h + pad_h)
                   * crop_size + w + pad_w;
          int top_index_2 = (((item_id * 2 + 1) * channels + c) * crop_size + h + pad_h)
                   * crop_size + w + pad_w;
          // int top_index = (c * height + h) * width + w;
          Dtype pixel_1 = static_cast<Dtype>(ptr_1[img_index++]);
          Dtype pixel_2 = static_cast<Dtype>(ptr_2[img_index++]);
          if (this->has_mean_file_) {
            int mean_index = (c * mean_height + h + y1 + pad_h)
                         * mean_width + w + x1 + pad_w;
            top_data[top_index_1] = (pixel_1 - mean[mean_index]) * scale;
            top_data[top_index_2] = (pixel_2 - mean[mean_index]) * scale;
          } else {
            if (this->has_mean_values_) {
              top_data[top_index_1] = (pixel_1 - this->mean_values_[c]) * scale;
              top_data[top_index_2] = (pixel_2 - this->mean_values_[c]) * scale;
            } else {
              top_data[top_index_1] = pixel_1 * scale;
              top_data[top_index_2] = pixel_2 * scale;
            }
          }
        }
      }
    ...
    // get window label
    const int num_labels = GenericWindowDataLayer::NUM - 5;
    top_label[item_id * num_labels] = 
        window[GenericWindowDataLayer<Dtype>::X_TRANS_LABEL];
    top_label[item_id * num_labels + 1] = 
        window[GenericWindowDataLayer<Dtype>::Z_TRANS_LABEL];
    top_label[item_id * num_labels + 2] = 
        window[GenericWindowDataLayer<Dtype>::Y_ROTAION_LABEL];
  }
  timer.Stop();
  batch_timer.Stop();
